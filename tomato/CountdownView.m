...
-(instancetype)initWithFrame:(CGRect)frame  totalTime:(NSInteger)totalTime lineWidth:(CGFloat)lineWidth  lineColor:(UIColor *)lineColor startBlock:(CountDownStartBlock)startBlock completeBlock:(CountDownCompleteBlock)completeBlock
{
    ...
    if (self) {
        ...
        countLabel.font = [UIFont fontWithName:@"Helvetica-Light" size:frame.size.width/6];
    ...
    if (_totalTime > 0) {
        ...
        dispatch_source_set_event_handler(_timer, ^{
            if(timeout<=0){ //倒计时结束，关闭
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if(timeout>=59){
                        self.countLabel.text = [NSString stringWithFormat:@"%@ 分 %@ 秒",@((timeout+1)/60).stringValue,@(seconds).stringValue];
                    }
                    else{
                        self.countLabel.text = [NSString stringWithFormat:@"%@ 秒",@(seconds).stringValue];
                    }
                });
                timeout--;
            }
